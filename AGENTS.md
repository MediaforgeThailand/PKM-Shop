...
side, goes through the one LINE OA.** The AI seller runs on the **Anthropic Messages API**
(Ready.md §2; default model `claude-sonnet-4-6`, changeable via `app_settings.ai_model` / env
`AI_MODEL`) with a fixed store-rules system prompt in `_shared/ai.ts` and a strict marker
protocol. Most of the "core" below protects two things: **the conversation/marker contract**
and **the integrity of money, stock, and payroll data**.
...
| Area | Files | Rule |
|---|---|---|
| AI chat contract | `supabase/functions/_shared/ai.ts` (`callSalesModel`) | Anthropic Messages API only (owner directive 2026-07-14, per Ready.md §2). Trust-channel isolation is load-bearing: the `system` param carries ONLY the fixed store rules + brand name; every customer-controlled value (catalog JSON, customer context, chat history, the message) rides in the user content. Model comes from `app_settings.ai_model` → env `AI_MODEL` → `claude-sonnet-4-6`; never hardcode a model elsewhere. Do not post-process model text beyond the marker parser. |
| Marker protocol | `supabase/functions/_shared/marker.ts` | `[[products: ≤4 ids]]`, `[[categories]]`, `[[order_status]]`, `[[handoff]]` — one marker max, final line, always stripped from visible text. New card type = prompt update in `ai.ts` + owner approval + regression tests in `_shared/__tests__/marker_test.ts`. |
...
2. **Gates stay green.** `npm run verify` at the repo root (web typecheck + build, `deno check`
   on every edge function, the Deno unit tests) must pass on every PR. Never weaken/skip/delete
   an assertion to make a build pass — fix the cause or stop.
...
5. **Live environment is owner territory.** Agents do not deploy edge functions, apply migrations
   ...
   (The 2026-07-14 v1.1 deploy to the dev project `mrygwthvyzrkxghgjimh` was owner-granted.)
...
6. **Thai-first UX.** Customer- and staff-facing strings are Thai; code/identifiers English.
   Rider/Packer screens are mobile-first, big tap targets. No browser `alert/confirm/prompt` —
   use the shared primitives in `web/src/lib/ui.tsx`.
7. **Compatibility.** `chat_messages`, the orchestrator response, and the action schema are
   consumed by the LINE webhook and the web app. Shape changes must be additive (deprecate,
   don't repurpose); the shared shapes live in `supabase/functions/_shared/pkmTypes.ts`.
