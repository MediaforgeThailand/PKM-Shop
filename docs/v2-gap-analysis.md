...
- `npm run v2:open-questions-audit`: passing, 28 unresolved-contract topics and 4 blocked rows checked.
- `npm run v2:local-readiness-audit`: passing, 0 Missing rows, 21 decision blockers, and 5 external gates checked.
- `npm run v2:docs-audit`: passing, 11 docs checked for stale verification evidence.
- `npm run v2:client-audit`: passing, 28 production files scanned.
- `npm run v2:edge-security-audit`: passing, 13 edge/shared/template files scanned.
- `npm run v2:health-safety-audit`: passing, 14 health/lab/wearable files scanned.
- `npm run types:mirror-audit`: passing, 34 exported types checked.
- `git diff --check`: passing, with Windows line-ending warnings only.
- `npx.cmd -y deno@2.8.2 test --allow-env --allow-net --import-map=supabase/functions/import_map.json supabase/functions/_shared/__tests__/`: passing, 68 tests.
- `npm run v2:deno-check`: passing for 7 v2 edge entrypoints.
- `SUPABASE_SHADOW_DB_URL`: not set locally, so `scripts/rls-check.sql` was not executed against a live shadow database in this run.
...
| Seed and RLS checks | Partial | `scripts/seed-demo.mjs`, `scripts/rls-check.sql`, CI optional shadow DB step | `scripts/rls-check.sql` now seeds test auth users and checks customer/catalog isolation, fact key read visibility, `user_facts`, append-only `consents`, tenant-member/staff/admin boundaries, and customer/staff/admin isolation for chat, orders/events, referrals/commissions, labs, and wearable metrics. Local/CI live execution still requires `SUPABASE_SHADOW_DB_URL`; secret/hard-fail policy is logged in `docs/v2-open-questions.md`. |
...
| Shared Deno modules and §10 unit coverage | Exists | `supabase/functions/_shared/*`, `_shared/__tests__/*`, `.github/workflows/miracare-v2.yml` import-map Deno step | `npx.cmd -y deno@2.8.2 test --allow-env --allow-net --import-map=supabase/functions/import_map.json supabase/functions/_shared/__tests__/` passes locally via npm cache with 68 tests. Tests cover marker's 8 spec cases plus unknown-key drop/log, PromptPay fixtures, state-machine legal/illegal transitions, commission calculation, referral attribution windows including zero/negative boundaries, context empty/full/overflow, fact normalizer Thai numerals/Buddhist year/kg parsing, lab, wearable, LINE token/signature/text/Flex carousel/QR image/payment postback helpers including underscore tenant env fallback and empty-product handling, lab-summary safety, lab vision normalization table, fixture-backed lab/wearable sample parsing, Apple Health XML chunk streaming, Apple Health zip `export.xml` streaming, and the internal service-role guard. |
| Published OpenAI prompt variables, `store:false`, no local system prompt layering | Exists | `supabase/functions/_shared/openai.ts`, `npm run chat:quality` assertions | `chat:quality` now asserts the customer chat screen does not import/use prompt-version reads or local prompt override paths; real prompt-content drift can only be checked in OpenAI Platform. |
...
| Chat order actions: select product, collect fields, form fallback, payment done | Exists | `_shared/orchestrate.ts`, `components/chat/OrderPanel.tsx`, `lib/ai/miraChat.ts`, `npm run v2:client-audit`, `npm run v2:edge-security-audit` | Action responses that skip the model now persist the user turn and a templated `system_notice` unless the transition RPC already inserted it; the live chat append uses the same `system_notice` role. E2E purchase against real Supabase still needed. |
| Admin order queue with transcript, actions, signed slip thumbnails | Exists | `app/admin/orders.tsx`, `components/admin/OrdersQueue.tsx` | Browser smoke reaches auth gate locally; signed thumbnails need real stored slips. |
| Slip upload / attaching `slip_url` | Blocked | `docs/v2-open-questions.md` Phase 3 | Spec lacks upload/write contract for private `payment-slips`. |
| Chat purchase E2E reaches admin queue and books | Partial | Components/functions exist; audits pass | Needs seeded Supabase integration test or manual run; persisted order-panel reload/QR recovery also needs the contract logged in `docs/v2-open-questions.md`. |
...
| Lab ingest: storage download, OpenAI extraction, confidence gate, summary once, facts for ready rows | Exists | `supabase/functions/lab-ingest`, `_shared/openai.ts`, `_shared/lab.ts`, `lib/templates.ts`, `supabase/functions/_shared/__tests__/fixtures/lab_vision_results.json`, `npm run v2:health-safety-audit` | Real image-to-OpenAI sample lab run is not locally proven. Lab summary storage is now deterministically sanitized to remove diagnosis wording and append the fixed disclaimer from `lib/templates.ts`. The vision call embeds the spec's 15-code normalization table in system text; broader raw-name aliases need owner confirmation. |
...
| Wearable ingest from Apple Health export zip/XML | Partial | `supabase/functions/wearable-ingest`, `_shared/wearable.ts`, `_shared/__tests__/wearable_test.ts`, `npm run v2:health-safety-audit` | `wearable-ingest` now streams storage objects, reads `export.xml` from Apple Health zip chunks with `fflate`, and still accepts extracted XML. Request shape still needs confirmation because the spec lists `{storage_path}` while the implementation requires tenant/customer context for scoped writes. |
...
| Schema contract audit in CI | Exists | `scripts/v2-schema-audit.mjs`, `package.json`, `.github/workflows/miracare-v2.yml` | `npm run v2:schema-audit` checks 16 v2 tables, 32 policies, 30 indexes, 29 numbered migrations, chat message idempotency, catalog-key immutability, legacy catalog consolidation, storage bucket contract, referrer type enum, and referral FK constraints. |
| Open-question contract audit in CI | Exists | `scripts/v2-open-questions-audit.mjs`, `docs/v2-open-questions.md`, `.github/workflows/miracare-v2.yml` | The audit keeps 28 unresolved-contract topics present in the open-question doc and checks that `Blocked` gap-analysis rows cite the authoritative list. |
...
| Deterministic local verification bundle | Exists | `npm run v2:verify`, `package.json`, `.github/workflows/miracare-v2.yml` | The bundle runs the local deterministic gates. Secret-backed chat regression, shadow RLS, and LINE sandbox checks remain separate because they require owner-provided external state. |
| External gate readiness preflight | Exists | `npm run v2:external-preflight`, `scripts/v2-external-preflight.mjs` | The preflight reports missing prerequisites for live seeding, chat regression, shadow RLS, OpenAI prompt-content review, and LINE sandbox checks without exposing secret values. It does not replace those external verification runs. |
| Standard edge response envelope for v2 functions | Exists | `_shared/http.ts`, v2 functions use `json()`/`toErrorResponse()`, `npm run v2:edge-security-audit`, `npm run v2:deno-check` | The edge audit now checks all v2 edge entrypoints for shared CORS/envelope helpers, rejects raw `new Response(...)`, and guards the LINE QR image reply path; CI also Deno-checks all 7 v2 edge entrypoints. Legacy utility functions (`rag-embed`, `openai-transcribe`) still use older response helpers and are outside the v2 replacement path. |
...
| Service-role tenant filtering in v2 edge functions | Partial | `npm run v2:edge-security-audit`, chat order actions now validate tenant/session/customer ownership before order field writes and `payment_done`; admin order actions load orders through the authenticated user's tenant allow-list; `fact-extractor` is service-role-only; fact/lab/wearable writes now include tenant filters where tenant context is available. | `docs/v2-open-questions.md` records unresolved contract gaps where the spec omits tenant context for the initial `fact-extractor` and `lab-ingest` lookups. |
