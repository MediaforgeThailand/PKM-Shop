...
- `npm run v2:local-readiness-audit`: passing, 0 Missing rows, 21 decision blockers, and 4 external gates checked.
...
| External gate readiness preflight | Exists | `npm run v2:external-preflight`, `scripts/v2-external-preflight.mjs` | The preflight reports four external gates waiting: live seeding, chat regression, shadow RLS, and LINE sandbox checks without exposing secret values. It does not replace those external verification runs. |
