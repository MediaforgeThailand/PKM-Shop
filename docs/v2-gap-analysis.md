...
- `npm run v2:open-questions-audit`: passing, 19 unresolved-contract topics and 3 blocked rows checked.
- `npm run v2:local-readiness-audit`: passing, 0 Missing rows, 17 decision blockers, and 4 external gates checked.
...
| Open-question contract audit in CI | Exists | `scripts/v2-open-questions-audit.mjs`, `docs/v2-open-questions.md`, `.github/workflows/miracare-v2.yml` | The audit keeps 19 unresolved-contract topics present in the open-question doc and checks that `Blocked` gap-analysis rows cite the authoritative list. |
