...
| Lab ingest: storage download, OpenAI extraction, confidence gate, summary once, facts for ready rows | Exists | `supabase/functions/lab-ingest`, `_shared/openai.ts`, `_shared/lab.ts`, `_shared/templates.ts`, `supabase/functions/_shared/__tests__/fixtures/lab_vision_results.json`, `npm run v2:health-safety-audit` | Real image-to-OpenAI sample lab run is not locally proven. Lab summary storage is now deterministically sanitized to remove diagnosis wording and append the fixed disclaimer from the canonical edge template file; `lib/templates.ts` is an app-side mirror. The vision call embeds the spec's 15-code normalization table in system text; broader raw-name aliases need owner confirmation. |
