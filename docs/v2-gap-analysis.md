...
- `npm run v2:type-safety-audit`: passing, 104 TypeScript files scanned.
- `npm run chat:quality`: passing.
- `npm run orders:status-audit`: passing.
- `npm run v2:schema-audit`: passing.
- `npm run v2:open-questions-audit`: passing, 24 unresolved-contract topics and 3 blocked rows checked.
- `npm run v2:local-readiness-audit`: passing, 0 Missing rows, 20 decision blockers, and 4 external gates checked.
...
- `npm run v2:edge-security-audit`: passing, 15 edge/shared/storage/template files scanned.
...
- `npx.cmd -y deno@2.8.2 test --allow-env --allow-net --import-map=supabase/functions/import_map.json supabase/functions/_shared/__tests__/`: passing, 79 tests.
...
| Chat purchase E2E reaches admin queue and books | Partial | Components/functions exist; audits pass | Needs seeded Supabase integration test or manual run; persisted order-panel reload/QR recovery still needs the contract logged in `docs/v2-open-questions.md`. |
...
| Open-question contract audit in CI | Exists | `scripts/v2-open-questions-audit.mjs`, `docs/v2-open-questions.md`, `.github/workflows/miracare-v2.yml` | The audit keeps 24 unresolved-contract topics present in the open-question doc and checks that `Blocked` gap-analysis rows cite the authoritative list. |
