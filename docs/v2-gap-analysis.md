...
- `npm run v2:e2e-commerce`: credentialed live commerce runner is available; it requires Supabase credentials plus a seeded demo tenant with `chk-basic` and `promptpay_id`.
...
- `npm run types:mirror-audit`: passing, 38 exported types checked.
...
- `npm run v2:deno-check`: passing for 8 v2 edge entrypoints.
...
| Commission creation on confirmed order | Exists | Phase 4 `transition_order(...)` replacement, unique `commission_entries.order_id`, `_shared/commissions.ts`, `_shared/__tests__/commissions_test.ts`, `scripts/e2e-commerce.mjs` | Unit coverage covers percent, flat, category override, rounding, and negative clamp behavior. The live runner also creates a server-generated referrer, passes `ref_code` through chat, confirms an attributed purchase, and asserts the commission amount matches `orders.commission_scheme_snapshot` once the external commerce E2E gate is runnable. |
...
| Lab ingest: storage download, OpenAI extraction, confidence gate, summary once, facts for ready rows | Exists | `supabase/functions/lab-ingest`, `_shared/openai.ts`, `_shared/lab.ts`, `_shared/templates.ts`, `supabase/functions/_shared/__tests__/fixtures/lab_vision_results.json`, `npm run v2:health-safety-audit` | Real image-to-OpenAI sample lab run is not locally proven. `lab-ingest` is service-role internal, derives tenant from `customer_id -> customers.tenant_id`, and does not accept request tenant fields. Lab summary storage is deterministically sanitized to remove diagnosis wording and append the fixed default disclaimer from the canonical edge template file; `lib/templates.ts` is an app-side mirror. Final legal wording sign-off remains `OWNER-REVIEW`. |
| Low-confidence lab confirmation UI/write path | Exists | `components/HealthInsightScreens.tsx`, `lib/health/labConfirm.ts`, `supabase/functions/lab-confirm`, `supabase/functions/_shared/labFacts.ts`, `npm run v2:client-audit`, `npm run v2:edge-security-audit`, `npm run v2:deno-check` | The lab results screen lists editable low-confidence/unconfirmed rows, calls the authenticated `lab-confirm` edge function, validates customer-owned `needs_confirmation` reports, marks confirmed rows, moves reports to `ready` after all low-confidence rows are confirmed, and inserts lab facts through the same shared helper used by `lab-ingest`. |
| Wearable ingest from Apple Health export zip/XML | Partial | `supabase/functions/wearable-ingest`, `_shared/wearable.ts`, `_shared/__tests__/wearable_test.ts`, `npm run v2:health-safety-audit` | `wearable-ingest` is service-role internal, derives tenant from `customer_id -> customers.tenant_id`, streams private `wearable-imports` storage objects, reads `export.xml` from Apple Health zip chunks with `fflate`, and still accepts extracted XML. Remaining gap is live proof. |
| Dashboard screens read only live lab/wearable/fact tables | Partial | `components/HealthInsightScreens.tsx`, `lib/health/v2HealthDashboard.ts`, `npm run v2:client-audit`, `npm run v2:health-safety-audit` | Client and health audits now assert the production health routes render `HealthInsightScreen`, the loader reads `lab_reports`, nested `lab_results`, `wearable_metrics`, and `user_facts`, and dashboard view/loader/routes do not use mock data or model calls. Real sample data visual proof is still pending. |
...
| Standard edge response envelope for v2 functions | Exists | `_shared/http.ts`, v2 functions use `json()`/`toErrorResponse()`, `npm run v2:edge-security-audit`, `npm run v2:deno-check` | The edge audit now checks all v2 edge entrypoints for shared CORS/envelope helpers, rejects raw `new Response(...)`, and guards the LINE QR image reply, payment-slip signed upload/read paths, and lab-confirm trusted write path; CI also Deno-checks all 8 v2 edge entrypoints. Legacy utility functions (`rag-embed`, `openai-transcribe`) still use older response helpers and are outside the v2 replacement path. |
...
See `docs/v2-open-questions.md` for the authoritative list. Current blockers are legal-approved lab disclaimer wording (`OWNER-REVIEW`) and LINE sandbox credentials.
