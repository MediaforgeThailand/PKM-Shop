...
The Chatbot tab is wired through the `mira-chat` Supabase Edge Function. Chat answers are generated by the published MiraCare prompt in OpenAI Platform, while the backend supplies personal context, recent chat, and the active hospital product catalog. The OpenAI API key stays on the backend as a function secret.
...
The helper deploys `mira-chat` with JWT verification enabled. Users must sign in through Supabase Auth before the mobile app can call OpenAI or save chat-derived health facts.

For local function testing:

```bash
supabase functions serve mira-chat --env-file .env.local
```
...
For custom hosting, set `EXPO_PUBLIC_AI_PROXY_URL` to your backend endpoint. If it is empty, the app calls `supabase.functions.invoke('mira-chat')`.
